<template>
  <div style="width: 100%">
    ...
    <div class="server_parameters_inputs">
      <ServerParameters
        :server_parameters="chart_data.getSearchParameters()"
        @update:server_parameters="updateServerParameters($event)"
      />
    </div>
    ...
    <DataTable
      ...
      paginator
      :rows="5"
      :globalFilterFields="table.fields"
      :filters="filters"
    >
      <Column
        ...
        :header="table.columnNames[index]"
        sortable
      ></Column>
    </DataTable>
  </div>
...
export default {
  ...
  methods: {
    async updateServerParameters(updatedValues) {
      const data = await new TableDataFactory().getInstance(
        this.chart_data,
        null,
        updatedValues,
      );
