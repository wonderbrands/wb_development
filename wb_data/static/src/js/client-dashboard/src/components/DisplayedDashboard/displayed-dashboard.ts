...
class DashboardData extends Dashboard {
  ...
  existComponentByTitle(title: string): boolean {
    const exists = this.getComponents().filter((x) => x.getTitle() === title);
    return exists.length > 0;
  }

  getComponentByTitle(title: string): ChartComponent | null {
    if (this.existComponentByTitle(title)) {
      const result = this.getComponents().filter((x) => x.getTitle() === title);
      return result[0];
    }
    return null;
  }
...
const test_dashboard_data: DashboardData[] = [
  new DashboardData(new Dashboard("Dashboard 1"), [
    new ChartComponent(
  ...
  new DashboardData(new Dashboard("Dashboard 2"), [
    new ChartComponent(
...
class DashboardDataFactory {
  ...
  convertServerComponent(data: any, dashboard: Dashboard, userInfo:UserInfo): DashboardData {
    ...
    let new_dashboard: DashboardData = new DashboardData(dashboard, components);
